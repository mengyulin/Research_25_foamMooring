...
使用時，將 [[./tools/prePostProcessing.ipynb][prePostProcessing.ipynb]] 複製到計算例之 ~background~ 資料夾，使用 Jupyter Notebook (或 VS Code) 開啟並逐步執行。此程式集可將 ~log.overInterDyMFoam~ 複製到 ~Plots~ 資料夾，以進行 3DoF 位移量之擷取。所繪圖形均存放於 ~Plots~ 資料夾。
