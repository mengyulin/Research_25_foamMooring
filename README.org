...
#+ATTR_HTML: alt: Alternative text title: Image Title :width 600
#+ATTR_LATEX: :placement [!tb] :width 0.8\textwidth
#+CAPTION: Chen and Hall (2022) 之模擬區域示意圖 (3D)。
#+NAME: fig:Chen_Hall_2022
[[./docs/img/Chen_Hall_2022.png]]
...
- 浮體幾何條件：見資料夾 ~tools/~ 中的 [[./tools/floating_body.ipynb][floating body.ipynb]]。
- 波浪條件：見資料夾 ~tools/~ 中的 [[./tools/DispersionEq.ipynb][DispersionEq.ipynb]]。
...
#+ATTR_HTML: alt: Alternative text title: Image Title :width 600
#+ATTR_LATEX: :placement [!tb] :width 0.8\textwidth
#+CAPTION: Case No. 41 計算結果。
#+NAME: fig:case41
[[./docs/img/case_41FFT_time_series.png]]
...
- 基本方程式: 參考 [cite/t:@chenCFDSimulationFloating2022] 之 Sec. 2 Methodology。
- 數值模式簡介: 包含了
  - OpenFOAM 簡介
  - 繫纜系統工具庫 foamMooring 簡介
  均可參考 [cite/t:@chenCFDSimulationFloating2022] 及 [cite/t:@chenCFDSimulationMultiple2024]。

*** 模式設置與驗證

- 模式設置與驗證，均可參考 [cite/t:@chenCFDSimulationFloating2022] 之 Sec. 3.2 Numerical model setup，但要注意本研究是二維模擬，而論文中為三維。

*** 成果分析

- 浮體 3DoF 位移、RAO 成果與波浪流場展示: 參考 [cite/t:@islamOpenFOAMAnalysisWave2019]。
- 纜繩張力分析: 參考 [cite/t:@chenCFDSimulationFloating2022] 及 [cite/t:@chenCFDSimulationMultiple2024]。

** 成果報告與海報範例

- 期末書面報告與海報 (poster) 的製作指引，見資料夾 [[./docs/report_tutorial][report tutorial]]。
- 書面報告可 Word 格式，可以根據碩士論文的樣式來修改 (可刪去誌謝、英文摘要等部分): [[./docs/report_tutorial/cycu_thesis_example.doc][碩士論文樣式檔]]。
- 過去學生之成果報告 (Word 檔) 與海報 (PPT 檔) 範例，見資料夾 [[./docs/example][example]]。
- 課程的 Notion 頁面也有許多參考資訊：[[https://pond-bass-4f5.notion.site/2025-1a474decdb7d806cbc7ac3cbafaa0b5f?source=copy_link][Notion page]]。

* 風力模組與計算

見頁面：[[./windLoad.org][windLoad]]。
