...
本專題以 **foamMooring** 工具箱為基礎，模擬單一矩形浮體（floating
body）在不同波浪條件下的運動響應。
藉由系統性地改變波浪週期與波高，分析浮體在三自由度運動（3-DoF） 下的
...
根據 Chen and Hall (2022)
的單一浮體案例，本研究採用相同的幾何、繫泊與水槽條件，改變波浪週期與波高兩個主要參數。

## 模擬區域設定

<figure id="fig:Chen_Hall_2022">
<img src="./docs/img/Chen_Hall_2022.png" />
<figcaption>Chen and Hall (2022) 之模擬區域示意圖 (3D)。</figcaption>
</figure>
...
- 浮體慣性矩 (I<sub>xx</sub>, I<sub>yy</sub>, I<sub>zz</sub>) =
  $(0.015, 0.015, 0.021)~\mathrm{kg\, m^2}$
- 靜止水深 $h = 0.5~\mathrm{m}$
...
- 繫纜導纜器 (fairlead, 與浮體連接處)
  $(x, y, z) = (\pm 0.1, \pm 0.1, -0.0736)~\mathrm{m}$
- 繫纜錨定處 (ahcnor, 與底床連接處)
  $(x, y, z) = (\pm 1.385, \pm 0.423, -0.5)~\mathrm{m}$
...
以相對水深 $(h/L)$ 來分類，以上波浪均屬中間水深波 (intermediate water
waves)。波浪尖銳度 (wave steepness) $H/L$
與波浪的非線性程度有關，但並非一個獨立的分類指標。相對水深之分類為：
...
- 浮體幾何條件：見資料夾 `tools/` 中的 [floating
  body.ipynb](./tools/floating_body.ipynb)。
- 波浪條件：見資料夾 `tools/` 中的
  [DispersionEq.ipynb](./tools/DispersionEq.ipynb)。

## 第一組 (基準案例)

依據 Chen and Hall (2022) 基礎模擬條件，設置靜水深 $h = 0.5~\mathrm{m}$,
浮體半寬 \$b = 0.1~m\$。
...
以波數 (wave number) $k = 2\pi/L$ 之倒數為長度之無因次尺度，其中 $L$
為波長。改變波浪週期 $T$ 可得到不同之 $k$ 值。 規劃相對之浮體寬為
$kb = 0.3 \sim 1.2$, 波浪尖銳度 $ka = 0.05, 0.1$, 其中 $a = H/2$
為入射波振幅。 計算例設定如下表：
...
增加浮體寬度，設定半寬 $b = 3d = 0.236~\mathrm{m}$,
其餘尺寸參數不變，靜水深仍為 $h = 0.5~\mathrm{m}$:
...
- 浮體吃水深與靜水深比 $d/h = 0.1572$
- 繫纜導纜器 (fairlead, 與浮體連接處)
  $(x, y, z) = (\pm 0.236, \pm 0.1, -0.0736)~\mathrm{m}$
...
規劃相對之浮體寬為 $kb = 0.3 \sim 1.2$, 波浪尖銳度 $ka = 0.05, 0.1$,
其中 $a = H/2$ 為入射波振幅。 計算例設定如下表：
...
<figure id="fig:case41">
<img src="./docs/img/case_41FFT_time_series.png" />
<figcaption>Case No. 41 計算結果。</figcaption>
</figure>
...
水深影響波的傳播速度與波壓分佈。若 $kh > \pi$ 為深水波；若 $kh < \pi/10$
為淺水波。 本研究之 \$h = 0.6~m\$，屬於中等深度（intermediate
depth），適合觀察深水與淺水效應交替之情況。
...
- Surge (沿 $x$ 方向之水平運動):
  $\left|\mathrm{RAO}_x \right| = \hat{x}/A_I$
- Heave (沿 $z$ 方向之垂直運動):
  $\left|\mathrm{RAO}_z \right| = \hat{z}/A_I$
- Pitch (沿 $y$ 方向之角度轉動，順時針為正):
  $\left|\mathrm{RAO}_\theta\right| = (\hat{\theta} \times b)/A_I$
...
註：Pitch RAO
的物理意義：將角度振幅乘以半寬，可換算為浮體兩端的升降位移振幅，再與入射波振幅相比。
...
對浮體各自由度時間序列（surge, heave, pitch）進行 FFT,
確認主頻是否與理論頻率一致。
...
在案例主目錄中執行 `./Allrun.pre`,
可執行平行計算。如需進行單核心計算，則將其修改為
...
在 `background` 資料夾中的 `FV.pvsm` 檔，為 ParaView 之 State
檔案，可以在 OpenFOAM 中使用 `load state`
選項開啟，即可得到已經設定好的繪圖頁面。
...
3.  欲分離出物體，需透過 `Threshold` 工具：
    ...
    a\. 從 `Pipeline Browser` 處，在 `aa.foam` 點選 `Threshold`. 出現
    `Threshold 1` 。在 `Threshold 1` 中的 `Properties` 之 `Scalars` 選擇
    `ZoneID`, 並設定其值之 `Minimum = 0`, `Maximum = 0` 。顯示此
    `Threshold 1` 並關閉 `aa.foam`, 可顯示出背景流體。

    b\. 在 `Threshold 1` 中再做一次 `Threshold` 工具，出現 `Threshold 2`
    。在 `Threshold 2` 處之 `Properties` 之 `Scalars` 選擇 `cellTypes`,
    設定值 `Minimum = 0`, `Maximum = 1` 。開啟 `Threshold 2` 並關閉
    `Threshold 1`, 可出現扣除浮體網格之背景流場。

    c\. 在 `aa.foam` 點選 `Threshold`, 出現 `Threshold 3` 。在
    `Threshold 3` 中的 `Properties` 之 `Scalars` 選擇 `ZoneID`,
    並設定其值之 `Minimum = 1`, `Maximum = 1` 。點開
    `Advanced Properties`, 在 `Transforming` 中設定
    `Translation = (0, -0.2, 0)` 。顯示此 `Threshold 3`
    可出現浮體附近的流場。

    d\. 僅開啟 `Threshold2` 與 `Threshold3`, 並在 `Orientation Axes`
    中點選 `Camera Paralle Projection`, 可出現浮體被挖空之波浪流場。
...
2.  在 `Pipeline Browser` 中點選 `mdv2_pt.vtk.pvd`, 使用 `Transforming`
    將纜線平移到與浮體一樣的位置。
...
3.  設定輸出路徑，例如輸出至新增目錄 `animation` 中:
    ...
    ParaView 會自動輸出連續編號的影格，如 `frame_0000.png`,
    `frame_0001.png`, …
...
若連續編號的影格檔案為 `frame_0000.png`, `frame_0001.png`, …,
執行以下指令產生影片：
...
若連續編號的影格檔案為 `frame.0000.png`, `frame.0001.png`, …,
則執行以下指令產生影片：
...
前、後處理之 Python 程式碼 (Jupyter Notebook)
[prePostProcessing.ipynb](./tools/prePostProcessing.ipynb)
可進行以下計算與繪圖：
...
使用時，將 [prePostProcessing.ipynb](./tools/prePostProcessing.ipynb)
複製到計算例之 `background` 資料夾，使用 Jupyter Notebook
開啟並逐步執行。此程式集可自動將 `log.overInterDyMFoam` 複製到 `Plots`
資料夾，並執行 `extractMulti.sh` 進行 3DoF
位移量之擷取。所繪圖形均存放於 `Plots` 資料夾。

程式碼放置位置：

1.  [prePostProcessing.ipynb](./tools/prePostProcessing.ipynb) 放置於
    `background/` 中。
2.  [extractMulti.sh](./tools/extractMulti.sh) 放置於
    `background/Plots/` 中。
...
如電腦中有安裝 Anaconda, 即可在 **Anaconda Navigator** 中開啟 Jupyter
Notebook。

Anaconda
的安裝教學，可參見此連結：https://simplelearn.tw/anaconda-3-intro-and-installation-guide/

Jupyter Notebook
的完整介紹，可參見此[連結](https://medium.com/ai-for-k12/jupyter-notebook-%E5%AE%8C%E6%95%B4%E4%BB%8B%E7%B4%B9-%E5%AE%89%E8%A3%9D%E5%8F%8A%E4%BD%BF%E7%94%A8%E8%AA%AA%E6%98%8E-846b5432f044)。
...
- 基本方程式與數值方法
  ...
  - 數值模式簡介
    - OpenFOAM 簡介
    - 繫纜系統工具庫 foamMooring 簡介 (Chen and Hall 2022, Che et al.
      2024)
- 模式設置與驗證
  - 模式設置
  - 模式驗證
...
- 基本方程式: 參考 \[cite/t:@chenCFDSimulationFloating2022\] 之 Sec. 2
  Methodology。
  - 數值模式簡介
    - OpenFOAM 簡介
    - 繫纜系統工具庫 foamMooring 簡介 (Chen and Hall 2022, Che et al.
